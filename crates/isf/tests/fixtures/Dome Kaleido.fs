/*{
  "DESCRIPTION": "Kaléidoscope pensé en azimut : répétition angulaire autour du zénith (avec miroir), motif fbm à domain-warp, modes nappes/bandes/anneaux, offset et échelle d'élévation, étincelles audio. Convention : bas de l'image = avant du dôme. Pack Sources Dome-Native.",
...
// ================================================================
// Dome Kaleido — kaléidoscope en azimut autour du zénith
// Repli N-fold (option miroir) de l'azimut, motif généré en 3D sur
// la sphère (pas de couture) : fbm + domain warp, 3 modes de motif.
// Audio : Bass -> pulsation zoom, Mid -> flow, High -> étincelles.
// ================================================================
...
void main() {
    ...
    // repli kaléidoscopique en azimut
    float az = azel.x + t * rotSpeed * TAU * 0.1;
    float n = float(segments);
    if (segments > 1) {
        az = azRepeat(az, n);
        if (mirrorFold) az = abs(az);
    }
    // transformation d'élévation (zoom + offset, pulsé par les basses) —
    // repli MIROIR dans [0, 90°] : un clamp saturerait au zénith où
    // domeDir devient indépendant de l'azimut (disque de couleur unie)
    float zoomFx = elScale * (1.0 + audioBass * audioAmount * 0.25);
    float el = HALF_PI - abs(mod((azel.y - radians(elOffset)) * zoomFx, PI) - HALF_PI);

    // motif 3D sur la sphère (continuité au zénith garantie)
    vec3 dir = domeDir(vec2(az, el));
    vec3 q = dir * patternScale;
    float flowFx = flowAmount * (1.0 + audioMid * audioAmount * 0.8);
    float ts = t * flowSpeed;
    vec3 warp = flowFx * 1.2 * vec3(fbm3(q + vec3(0.0, 0.0, ts)) - 0.5,
                                    fbm3(q + vec3(5.2, 1.3, ts)) - 0.5,
                                    fbm3(q + vec3(9.7, 4.1, ts * 0.7)) - 0.5);
    vec3 qw = q + warp * 2.0;

    float f;
    if (patternMode == 0) {
        f = fbm3(qw + vec3(0.0, 0.0, t * 0.15));
    } else if (patternMode == 1) {
        // bandes : ondes selon l'azimut replié
        f = 0.5 + 0.5 * sin(az * n * 1.5 + fbm3(qw) * 5.0 + t * 0.5);
    } else {
        // anneaux : ondes selon l'élévation
        f = 0.5 + 0.5 * sin(el * 9.0 + fbm3(qw) * 5.0 - t * 0.8);
    }
    f = pow(clamp(f, 0.0, 1.0), contrast);

    vec3 col = palette4(fract(f * 1.15 + t * colorCycle * 0.2),
                        colorA, colorB, colorC, colorD, colorCount, paletteMode);
    // creuse les vallées, garde des hautes lumières franches
    col *= 0.15 + 1.45 * pow(f, 1.35);

    // étincelles haute fréquence pilotées par les aigus
    float sparkAmt = audioHigh * audioAmount;
    if (sparkAmt > 0.001) {
        float sp = hash21(floor(uv * 220.0) + floor(t * 12.0) * 0.37);
        col += step(0.995 - sparkAmt * 0.02, sp) * 1.5;
    }

    vec3 outCol = col * intensity;
    float alpha = clamp(max(bgAlpha, max(outCol.r, max(outCol.g, outCol.b))), 0.0, 1.0);
    float mask = domeMask(uv, maskFeather);
    gl_FragColor = domeOutput(outCol, alpha, mask);
}
